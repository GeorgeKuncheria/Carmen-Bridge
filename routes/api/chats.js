...
router.get("/:chatId", async (req, res, next) => {
  Chat.findOne({
    _id: sanitize(req.params.chatId),
...
router.put("/:chatId", async (req, res, next) => {
  Chat.findByIdAndUpdate(sanitize(req.params.chatId), sanitize(req.body))
...
router.get("/:chatId/messages", async (req, res, next) => {
  Message.find({ chat: sanitize(req.params.chatId) })
    .populate("sender")
    .then((results) => res.status(200).send(results))
    .catch((error) => {
      console.log(error);
...
router.put("/:chatId/messages/markAsRead", async (req, res, next) => {
  Message.updateMany(
    { chat: sanitize(req.params.chatId) },
